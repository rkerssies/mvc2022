...
//	use lib\db\mysqliDB;
	...
	class ModelBasic
	{
		...
		public function all()
		{
			...
			//TODO with published num_rows
			return $this->dbObject->QueryBindParams($stmt);
		}
		...
		{   // this method is at the end of each method-chain on a Model (not find or all) to send the query to the server
			...
			$allData =  $this->dbObject->querySQL($this->queryString);// original OK wit strait sql-statement
		...
		public function insert($dataArray)
		{
			...
			$result = $this->dbObject->querySQL('INSERT INTO `'.$this->table.'` ('.$fieldnames.') VALUES ('.$values.')');
			$this->inserted_id = $this->dbObject->insert_id;
			return $result;
		}

<?php
	...
	use lib\db\mysqliDB;
	use core\Request;
	use core\FormRequests;      // validator used in controller.
	use models\Fruit;
...
//			dd($request->all());        // example to see all request-data with Request-instance
		...
		public function index2(Fruit $fruit)
		{
			...
			dd("data-dump on url-path: /fruit2<br> with route-Middlweare 'rbac' thah has params <br>"
				.json_encode($fruit->all()));
		}
		...
		public function update(Request $request, FormRequests $validator, $id)
		{
			...
			elseif(empty($_POST)){ 			// geen submit, dan select=sql --> gekregen waarden in POST zetten
		...
		public function delete(Request $request, $id)
		{
			$request->all();
			if(is_numeric($id)
				&& $this->dbClass->querySQL('DELETE FROM `fruits` WHERE `id` = '.$request->get->p1))
			{
				redirect("/fruits/var_value3/var_value4");      // redirect
